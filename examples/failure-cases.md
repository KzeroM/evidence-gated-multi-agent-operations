...
| Non-independent reviewer | A medium- or high-risk review reuses the worker runtime or worker execution reference, or has write access | validation failure; no admissible verdict |
...
These outcomes are checked in negative end-to-end tests, including bad references, stale evidence, changed subjects, and non-independent medium/high-risk review.
...
- **Non-independent review:** a fictional medium-risk change packet reports the same runtime and execution reference for worker and reviewer and grants write access. Cross-document validation rejects the review before its verdict can be admitted; high risk follows the same rule. A live `read_only` view or immutable `offline_packet` with `read_only: true` is admissible.
