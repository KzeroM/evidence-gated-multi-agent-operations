...
- 중간 및 높은 위험에서는 작업자와 다른 리뷰어 런타임/실행 및 읽기 전용 접근이 필수입니다.
...
v2는 의도적인 호환 중단입니다. 형태 기반 Markdown 분기, 기준 문장 복사, 자유 형식 부작용/증거, 버전 없는 리뷰 및 `PASS_WITH_DEFERRALS`는 허용되지 않습니다. 문서 종류/버전을 추가하고, 안정적 ID를 배정하고, 부작용과 증거를 형식화하고, 불변 대상과 유효 기간을 추가하고, 실행 기록 및 리뷰 출처를 작성합니다. 모든 기준이 충족된 경우에만 `PASS`와 구조화된 `deferrals`를 함께 사용합니다. 역사적 보존이 필요하면 v1 아카이브를 별도로 유지합니다. 기존 v2 문서 형태는 호환되지만 이제 중간 위험 패킷도 높은 위험에 이미 적용되던 런타임·실행·읽기 전용 리뷰어 독립성 검사를 충족해야 합니다.
