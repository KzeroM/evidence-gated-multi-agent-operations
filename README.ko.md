...
- 중간 및 높은 위험에서는 작업자와 다른 리뷰어 런타임/실행이 필수입니다. 리뷰 접근은 `read_only: true`여야 하며 `access_mode`는 실시간 `read_only` 또는 불변 `offline_packet`을 사용할 수 있지만 `read_write`는 거부됩니다.
- 낮은 위험은 정책 기반 또는 논리적 리뷰어 분리를 유지할 수 있지만 리뷰 출처는 공개해야 합니다.

리뷰 출처에는 리뷰어 신원, 런타임/모델, 분리된 실행 참조, 컨텍스트 출처/범위, 접근 모드, 충돌 및 읽기 전용 상태가 포함됩니다. 검증기는 미션·실행·리뷰 기록 전체에서 중간/높은 위험 독립성 규칙을 검사하지만 리뷰어를 실행하거나 격리하지는 않습니다. 허용 부작용은 파일 읽기/쓰기 glob, 네트워크 도메인, 배포, 데이터베이스 변경 및 메시징 플래그로 표현합니다. 실제 강제는 주변 환경의 책임이며 프로토콜은 권한을 검사 가능하게 만듭니다.

`offline_packet`은 실시간 쓰기 표면을 노출하지 않으므로 프로토콜은 불변 패킷을 실시간 읽기 전용 리뷰와 같거나 더 강하게 격리된 것으로 취급합니다. 검증기는 함께 기록된 `read_only` 주장만 요구하며 패킷의 불변성이나 런타임 접근을 직접 입증하거나 강제할 수 없으므로, 주변 리뷰 시스템이 해당 통제를 제공하고 증명해야 합니다.
...
패키지 스모크 게이트는 sdist와 wheel을 모두 빌드하고 각각 격리된 환경에 새로 설치한 다음 소스 체크아웃 외부에서 설치된 `egmo`를 실행합니다. 경고 증명은 완전한 설치 템플릿을 검증하면서 `RefResolver` 사용 중단 경고만 오류로 승격하므로 취약한 패키지 전체 경고 정책을 피합니다.
...
구현 릴리스 2.1.0은 문서 `schema_version: "2.0"`과 기존 v2 형태를 유지합니다. 중간 위험 패킷에는 높은 위험에 이미 적용되던 런타임·실행·읽기 전용 리뷰어 독립성 검사를 적용합니다. 또한 `read_only: true`에서 실시간 `read_only` 또는 불변 `offline_packet`을 사용할 수 있음을 명확히 하며, 중간/높은 위험의 `read_write` 접근은 계속 유효하지 않습니다.
