...
POLICY_PATH = ROOT / "sanitization-policy.yaml"
...
def validate_chain(documents: Iterable[LoadedDocument], as_of: datetime | None = None) -> list[str]:
    ...
    for review in reviews.values():
        ...
        for record_id in data["evidence_record_ids"]:
            if record_id not in evidence_records:
        ...
        if data["verdict"] == "PASS":
            ...
            for record_id in data["evidence_record_ids"]:
                ...
                if record:
                    for item in record.data["items"]:
                        if check_time > _parse_time(item["expires_at"]):
                            errors.append(f"{review.label}: evidence {item['evidence_id']} is stale")
    for report in by_kind["final_report"]:
        ...
        if mission is None or execution is None or review is None:
            ...
            continue
        expected = VERDICT_TO_STATE[review.data["verdict"]]
        if data["judgment"] == "PASSED" and expected != "PASSED":
            errors.append(f"{report.label}: PASSED judgment requires PASS review")
        if data["judgment"] == "PASSED" and execution.data["current_state"] != "PASSED":
            errors.append(f"{report.label}: PASSED judgment requires PASSED execution state")
